"""
Dashboard Financiero Personal — Claudio Socias Paradiz v3
Cambios v3:
  - amort_deuda_tc / pago_minimo_tc (renombrado desde cuota_lca/cuota_tc)
  - Treemap reemplazado por barras horizontales agrupadas
  - Gráfico deuda con proyección liquidación septiembre 2026
...
# Datos deuda real del estado de cuenta marzo 2026
SALDO_CAPITAL_CLP  = 6520310
CUOTA_MENSUAL_CLP  = 283327
CUOTA_ACTUAL       = 8
TOTAL_CUOTAS       = 35
LIQUIDACION_MES    = 9   # septiembre 2026
...
def cargar_deuda_mensual():
    ...
    # Saldo real desde estado de cuenta + proyección futura
    tc_actual = 1070.0
    saldo_eur_actual = SALDO_CAPITAL_CLP / tc_actual
    cuota_eur = CUOTA_MENSUAL_CLP / tc_actual

    # Reconstruir saldo histórico hacia atrás desde saldo actual
    n = len(df)
    saldos = [0.0] * n
...
ESTILO_SEP = {"borderTop": f"1px solid {GRID}", "margin": "8px 0 16px 0"}
...
app.layout = html.Div([
    ...
    # Sección 3: Barras por categoría
    html.Div([
...
def cb_kpis(anio, trim, mes):
    ...
    bal  = ti - tg
    # Meses restantes hasta liquidación sep 2026
    hoy = date.today()
    ...
    return html.Div([
        ...
        kpi("Saldo deuda TC",     f"€{saldo_eur:,.0f}",
            f"Liquidación en {meses_liq} meses (sep 2026)", NARANJA),
...
def cb_gasto(anio, trim, mes):
    ...
    fig.add_hline(y=2200, line_dash="dash", line_color=DORADO, opacity=0.7,
...
def cb_barras(anio, trim, mes):
    ...
    if dff.empty:
        return layout_base(go.Figure(), "📊 Gastos por Categoría",
                           height=420)

    # Agrupar por categoría padre y subcategoría
    grp = dff.groupby(["categoria_padre","subcategoria"])["eur"].sum().reset_index()
    grp = grp[grp["eur"] > 0].sort_values("eur", ascending=True)

    cat_colors = {
        "alimentacion":  AZUL_C,
        "transporte":    VERDE,
        "salud_deporte": NARANJA,
        "servicios":     DORADO,
        "alojamiento":   GRIS,
        "comercio":      PURPURA,
    }

    # Una traza por categoría padre
    fig = go.Figure()
    for cat in grp["categoria_padre"].unique():
        ...
        fig.add_trace(go.Bar(
            ...
            orientation="h",
            name=cat,
            marker_color=cat_colors.get(cat, GRIS),
...
def cb_poder(anio, trim, mes):
    ...
    cats       = ["Supermercado","Restaurantes","Taxi/Uber",
                  "Transporte","Gimnasio","Suscripciones","Alquiler"]
    base_chile = [272, 146, 25, 34, 41,  4,  887]
    base_mad   = [250,  57, 25, 50, 67, 55, 1500]
    ...
    fig.add_trace(go.Bar(
        name="Salamanca (aj. IPC)", x=cats, y=madrid_aj,
        marker_color=NARANJA, opacity=0.9,
        hovertemplate="<b>%{x}</b><br>Salamanca: €%{y:,.0f}/mes<extra></extra>",
    ))
    layout_base(fig, "🏠 Poder Adquisitivo: Las Condes vs Salamanca")
    fig.update_layout(
...
def cb_salarial(anio, trim, mes):
    ...
    fig = go.Figure()
    for i, (nivel, smin, smax, color) in enumerate(
            zip(niveles, sal_min, sal_max, colores)):
        fig.add_trace(go.Bar(
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=14, color=color,
                       symbol="star" if i==2 else "circle"),
...
def cb_adv_tc_cat(anio, trim, mes):
    ...
    cat_colors = {
        "seguro_salud": ROJO, "restaurante": AZUL_C,
        "plataforma_pago": GRIS, "supermercado": VERDE,
        "kinesiologia": NARANJA, "delivery": DORADO,
    }
    colores = [cat_colors.get(s, ACENTO) for s in df["subcategoria"]]
    fig = go.Figure(go.Bar(
        x=df["eur"], y=df["subcategoria"],
        orientation="h",
        marker_color=colores,
        hovertemplate="<b>%{y}</b><br>€%{x:,.0f}<extra></extra>",
    ))
    layout_base(fig, "💳 Top 10 Categorías Gasto TC (histórico)")
    fig.update_layout(xaxis_title="EUR total", yaxis_title="",
                      showlegend=False)
    return fig
...
def cb_adv_ahorro(anio, trim, mes):
    ...
    merged = dff_ing.merge(
        dff_cc[["fecha","gastos_cc"]], on="fecha", how="left"
    ).merge(
        dff_tc[["fecha","gastos_tc"]], on="fecha", how="left"
    ).fillna(0)
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=merged["label"], y=merged["tasa"],
        marker_color=colores, name="Tasa ahorro",
    ...
    fig.update_layout(yaxis_title="% de ahorro", xaxis_tickangle=45,
                      showlegend=False)
    return fig
...
def cb_adv_top10(anio, trim, mes):
    ...
    fig = go.Figure(go.Bar(
        x=df["total_eur"], y=df["nombre"],
        orientation="h",
        marker_color=colores,
        customdata=df[["subcategoria","n"]].values,
...
def cb_adv_proyeccion(anio, trim, mes):
    ...
    # Proyección escenario A — seguir en cuotas
    fechas_a, saldos_a = [], []
    ...
    fig.add_trace(go.Scatter(
        x=fechas_a, y=saldos_a,
        mode="lines", name="Escenario A: seguir cuotas",
        line=dict(color=ROJO, width=2, dash="dash"),
        yaxis="y2",
        hovertemplate="<b>%{x}</b><br>Saldo proyectado: €%{y:,.0f}<extra></extra>",
    ))

    # Proyección escenario B — liquidar sep 2026
    fechas_b, saldos_b = [], []
    ...
    fechas_b.append(f"Sep {LIQUIDACION_ANIO}")
    saldos_b.append(0)

    fig.add_trace(go.Scatter(
        x=fechas_b, y=saldos_b,
        mode="lines+markers", name="Escenario B: liquidar sep 2026",
    ...
    ahorro_intereses = 725
    fig.add_annotation(
        x=f"Sep {LIQUIDACION_ANIO}", y=0,
        text=f"<b>Liquidación</b><br>Ahorro: €{ahorro_intereses:,}",
    ...
    fig.update_layout(
        ...
        yaxis_title="EUR / mes",
        yaxis2=dict(
            title=dict(text="Saldo pendiente (EUR)", font=dict(color=ACENTO)),
